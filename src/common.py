...
def _get_intersection_size(choice1: Choice, choice2: Choice) -> int:
...
def get_next_game_state(game_state: GameState, choice: Choice) -> GameState:
    intersection_size = min(map(lambda answer: _get_intersection_size(choice, answer), game_state.answers_remaining))

    # player picked a correct choice
    if intersection_size == 4:
        removed_choices = filter(lambda c: _get_intersection_size(c, choice) > 0, game_state.choices)
    ...
    if intersection_size == 3:
        removed_choices = filter(lambda c: _get_intersection_size(c, choice) == 2, game_state.choices)
        new_choices = game_state.choices.difference(removed_choices)
